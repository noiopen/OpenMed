...
const getClosestFacilities = require('../../src/index')
...
describe('getClosestFacilities unit tests', function () {
  it('should return the param errors', async () => {
    ...
    await expect(getClosestFacilities.main(params)).rejects.toThrow(
      'The input params are invalid: longitude or latitude unavailable'
    )
    ...
    await expect(getClosestFacilities.main(params)).rejects.toThrow(
      'The input params are invalid: facilities unavailable'
    )

    params = { latitude: 40.456221, longitude: -75.63128, facilities: [] }
    await expect(getClosestFacilities.main(params)).rejects.toThrow(
      'The input params are invalid: facilities are empty'
    )
  })
